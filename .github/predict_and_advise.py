...
def _clean_weather(s: str | None) -> Dict[str, Any]:
    if not s:
        return {}
    out = {"raw": str(s)}
    # 例: "Clouds, 24°C" / "晴 27C"
    m = TEMP_C.search(str(s))
    if m:
        try:
            out["temp_c"] = float(m.group(1))
        except Exception:
            pass
    out["label"] = str(s).split(",")[0].strip()
    return out
...
def extract_features(record: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for canon, alias_list in ALIASES.items():
        ...
        if canon in ("avg_hr", "min_hr", "max_hr", "steps"):
            val = _to_float(val)
        elif canon == "weather":
...
def _enrich_weather_tips(weather: Dict[str, Any] | None, tips: List[str]) -> List[str]:
    ...
    if "rain" in label or "drizzle" in label or "shower" in label or "storm" in label:
...
def _fallback_rules(feat: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "max_hr" in feat and feat.get("avg_hr"):
        if feat["max_hr"] and feat["avg_hr"] and feat["max_hr"] > feat["avg_hr"] + 40:
...
def _model_predict(feat: Dict[str, Any], model_path: str) -> tuple[str | None, List[str]]:
    """
