...
def build_features(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    （如果外部 make_advice 有更完整抽取，這裡只是 fallback）
    """
    f: Dict[str, Any] = {}

    # steps
    if "steps" in raw:
        f["steps"] = to_int(raw.get("steps"))
...
def write_advice(doc_ref, advice: Dict[str, Any], write_subdoc: bool, dry=False):
    """
    advice: 「advice.*」內容本體（不含 _advised 等）
    """
    payload = {
        "advice": advice,
        "_advised": True,
        ...
        "_version": VER,
        "_last_run": firestore.SERVER_TIMESTAMP,   # 心跳欄位，方便你看 pipeline 有跑
    }
    if dry:
...
def _parse_date_id(did: str) -> Optional[datetime]:
    ...
    for fmt in ("%Y-%m-%d", "%Y/%m/%d", "%Y%m%d"):
        try:
            return datetime.strptime(did, fmt).replace(tzinfo=timezone.utc)
        except Exception:
            continue
    return None
...
def main():
    ...
    start_dt = now - (timedelta(hours=args.since_hours) if args.since_hours and args.since_hours > 0
                      else timedelta(days=args.days))

    root = db.collection(args.root)

    # 收集候選日期（無法解析的也納入，但排在最後）
    ...
    for dd in date_docs:
        ...
        if ts is None:
            # 解析不了 → 仍納入（避免漏處理）
            if args.verbose:
                print(f"[{VER}] WARN: date id not parseable -> include: {dd.id}")
            candidates.append((datetime.max.replace(tzinfo=timezone.utc), dd))
        else:
            if ts >= start_dt:
                candidates.append((ts, dd))
    candidates.sort(key=lambda x: x[0])  # 由舊到新；無法解析的放最後

    seen = wrote = 0
    for ts, date_doc in candidates:
        date_id = date_doc.id
        print(f"[{VER}] DATE {date_id}")

        # 取 records，按 ID 排序（假定 HH:MM:SS；若不是也能固定順序）
        ...
        for r in recs:
            ...
            miss = [k for k in ["steps", "avg_heart_rate", "max_heart_rate", "temp_c"] if k not in feat]
            if args.verbose:
                print(f"[{VER}]    features={feat} missing={miss}")

            # 判斷是否完全不可用（feat 完全空）
            if not feat:
                if args.verbose:
                    print(f"[{VER}]    SKIP: no_usable_features (after normalization)")
                if not args.dry_run:
                    r.set(
                        {
                            "_advice_skip": "no_usable_features",
                            "_raw_keys": sorted(list(data.keys())),
                            "_last_run": firestore.SERVER_TIMESTAMP,
                            "_version": VER,
                        },
                        merge=True,
                    )
            ...
            # 先試外部 make_advice（含模型/規則），失敗就 fallback
            try:
                if _HAVE_MAKE_ADVICE:
                    ...
                    raise RuntimeError("no external make_advice")
