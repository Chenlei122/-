...
def get_date_range(days):
    ...
    # 這裡刻意多抓幾天，確保能涵蓋到未來的測試資料 (如 2025-12-25)
...
def main():
    ...
    # 2. 如果沒找到，嘗試「深層探測」(Collection Group Query on 'records')
    if not user_ids:
        print("👻 方法 A 找不到人，切換為方法 B: 深度反向搜尋 'records'...")
        try:
            # 搜尋最底層的 'records' 集合，這是唯一確定存在的實體
            blobs = list(db.collection_group("records").limit(50).stream())
            found_set = set()
            print(f"   🔍 掃描到 {len(blobs)} 筆底層數據，正在反推使用者...")
            
            for b in blobs:
                # b.reference.path 類似: users/UID/health_data/DATE/records/TIME
                path_parts = b.reference.path.split("/")
                if "users" in path_parts:
                    # 找到 "users" 之後的下一個就是 UID
                    idx = path_parts.index("users")
                    if idx + 1 < len(path_parts):
                        uid = path_parts[idx + 1]
                        found_set.add(uid)
    ...
    if not user_ids:
        print("❌ 錯誤: 真的找不到任何使用者。")
        print("   這通常代表你的 'records' 集合裡完全沒有資料，或者權限被拒絕。")
        return
